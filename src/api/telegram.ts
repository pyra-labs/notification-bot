...
export const bot = new Bot(appConfig.tgApiKey);
...
bot.on("message:text", async (ctx) => {
	...
	if (!walletRegex.test(ctx.message.text)) {
		ctx.reply(
			"That doesn't look like a valid wallet address... Please send me a valid Solana wallet address for me to start monitoring your account health.",
		);
		return;
	}
