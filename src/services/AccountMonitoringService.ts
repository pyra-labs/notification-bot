...
export class AccountMonitoringService {
	...
	constructor(driftClientManager: DriftClientManager) {
		...
		this.supabase = createClient(appConfig.supaBaseUrl, appConfig.supaBaseKey);
	...
	private async loadStoredAccounts(): Promise<void> {
		try {
			...
			const { data: accounts, error } = await this.supabase.from("monitored_accounts").select("*");
	...
	async startMonitoring(address: string, vaultAddress: string, chatId: number, intervalMs: number = 60000): Promise<void> {
		...
		try {
			...
			if (result === "new") {
				await bot.api.sendMessage(
					chatId,
					`I've started monitoring your Quartz account health! I'll send you a message if it drops below 25%, and another if it drops below 10%. Be sure to turn on notifications in your Telegram app to receive alerts! 🔔`,
				);
	...
	async stopMonitoring(address: string): Promise<void> {
		...
		try {
			...
			const { error } = await this.supabase.from("monitored_accounts").delete().eq("address", address);
	...
	private async _checkHealth(address: string, vaultAddress: string): Promise<void> {
		...
		try {
			...
			if (monitoring.lastHealth > 25 && currentHealth <= 25) {
				...
				await bot.api.sendMessage(
					monitoring.chatId,
					`Your account health for wallet ${walletDisplayAddress} has dropped to ${currentHealth}%. Please add more collateral to your account to avoid liquidation!`,
				);
			}
			...
			if (monitoring.lastHealth > 10 && currentHealth <= 10) {
				...
				await bot.api.sendMessage(
					monitoring.chatId,
					`🚨 Your account health for wallet ${walletDisplayAddress} has dropped to ${currentHealth}%. Add more collateral to your account now to avoid liquidation!`,
				);
			}
			...
			const { error } = await this.supabase.from("monitored_accounts").update({ last_health: currentHealth }).eq("address", address);
	...
	async getWalletAddressByChatId(chatId: number): Promise<string | undefined> {
		const { data, error } = await this.supabase.from("monitored_accounts").select("address").eq("chat_id", chatId).single();
	...
	async getMonitoredAccounts(): Promise<string[]> {
		const { data, error } = await this.supabase.from("monitored_accounts").select("address");

		if (error || !data) return [];
		return data.map((account: any) => account.address);
	}

	private async updateMonitoredAccount(
		address: string,
		vaultAddress: string,
		chatId: number,
		initialHealth: number,
	): Promise<"existing" | "new" | any> {
		const { data: existingEntry } = await this.supabase.from("monitored_accounts").select().eq("address", address).single();
		...
		try {
			...
			await this.supabase.from("monitored_accounts").insert({
				address: address,
				vault_address: vaultAddress,
				chat_id: chatId,
				last_health: initialHealth,
